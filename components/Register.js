        ...
        <div className="w-fit py-40 bg-cor-4/70 px-14 sm:py-14 shadow-md sm:rounded-3xl">
          <div className="flex flex-col items-center text-center mb-8 justify-center">
            ...
            <p className="flex items-baseline gap-x-2">
              <span className="text-5xl font-bold tracking-tight text-cor-0">Registro</span>
            </p>
          </div>
          <form className="flex flex-col text-sm rounded-md "onSubmit={handleSubmit}>
            <input className="bg-cor-4 h-auto w-full mb-5 rounded-[4px] p-3 hover:outline-none focus:outline-none" type="email" placeholder="Email" value={email} onChange={(e) => setEmail(e.target.value)} required/>
            <input className="bg-cor-4 h-auto w-full mb-5 rounded-[4px] p-3 hover:outline-none focus:outline-none" type="password" placeholder="Senha" value={password} onChange={(e) => setPassword(e.target.value)} required/>
            <input className="bg-cor-4 h-auto w-full sm:w-96 rounded-[4px] p-3 hover:outline-none focus:outline-none" type="password" placeholder="Confirmar Senha" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} required/>
            <button className="mt-5 w-full font-semibold rounded-3xl p-2 bg-gradient-to-r from-cor-2 bg-cor-1 text-cor-4 hover:bg-cor-3 scale-105 transition ease-in-out duration-300" type="submit">Registrar</button>
            {error && <p>{error}</p>}
          </form>
          <div className="mt-3 flex justify-between text-sm text-cor-0/70 hover:text-cor-1 transition ease-in-out duration-300">
            <a href="/">Esqueceu a senha?</a>
          </div>
          <p class="flex justify-center mt-3 text-sm text-cor-0/70">Continuar com</p>
